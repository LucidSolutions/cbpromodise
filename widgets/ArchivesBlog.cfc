...
component extends="contentbox.models.ui.BaseWidget" singleton{
	...
	ArchivesBlog function init(){
		// Widget Properties
		setName( "ArchivesBlog" );
		setVersion( "1.0" );
	...
	*/
	...
	private function buildList(archives,showPostCount){
		...
		saveContent variable="rString"{
			...
			for(var x=1; x lte arrayLen( arguments.archives ); x++){
				var thisDate = arguments.archives[ x ]["year"] & "-" & arguments.archives[ x ]["month"] & "-1";
				writeOutput('<li><i class="fa fa-calendar"></i> <a href="#cb.linkArchive(year=arguments.archives[ x ]['year'],month=arguments.archives[ x ]['month'])#">#dateFormat(thisDate,"mmmm yyyy" )#');
				if( arguments.showPostCount ){ writeOutput( " (#arguments.archives[ x ]['count']#)" ); }
				writeOutput('</a></li>');
			}
			// close ul
